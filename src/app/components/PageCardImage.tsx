...
const PageCardImage = ({ post }: Props) => {
    const { title, description, slug, image, createdAt } = post;
    const { url, width, height } = image.formats.medium;

  return (
    <div className=" bg-white border border-gray-200 rounded-lg shadow-sm mt-5 mb5 dark:bg-gray-800 dark:border-gray-700">
    <Link href={`/blog/${slug}`}>
              <Image
